...
# ==========================================
# 1. VISUALIZATION FUNCTIONS (HELPER)
# ==========================================
...
def display_results(results, y_test, y_pred):
    ...
    if "top_features" in results:
        ...
        with cp:
            ...
            st.write(", ".join([f"{w}" for w, c in results["top_features"]["positive"]]))
        with cn:
            st.error("**Negative Indicators:**")
            st.write(", ".join([f"{w}" for w, c in results["top_features"]["negative"]]))
            
    with st.expander("📋 Detailed Classification Report"):
        st.dataframe(pd.DataFrame(report).transpose().round(3), use_container_width=True)

# ==========================================
# 2. MAIN LOGIC (Class Weight Version)
# ==========================================
...
def run(input_path, output_dir):
    ...
    tfidf = TfidfVectorizer(
        ngram_range=(1, 3),   # Menangkap frase 3 kata
        max_df=0.90,          # Abaikan kata terlalu umum
        min_df=2,             # Abaikan typo unik
        stop_words=None,      # PENTING: Jangan hapus "not"/"no"
        sublinear_tf=True
    )
    ...
    # 4. Model Training (Class Weight)
    model = LogisticRegression(
        max_iter=3000,
        class_weight="balanced", # Mengatur bobot agar adil
        solver="lbfgs",
        C=2.0,                   # Regularization agak ketat
        random_state=42,
        n_jobs=-1
    )
